...
class ResBlock(nn.Module):
    def __init__(self, filter_size, n_cnn, n_resblock):
        nn.Module.__init__(self)
        ...
        self.rnn = nn.RNNCell(filter_size*n_cnn*n_cnn, filter_size*2) # input is size of self.conv output , hidden state is size of h output of GRU i.e. 1,N,H --> output is N, conv_hidden * 2 (same as output of self.film)
    ...
    def forward(self, input, idx, lin_h):
        ...
        res_lin = self.res_lins[idx]
        rnn_inp = res_lin(res_out) # N,2048
        lin_h = self.rnn(rnn_inp, lin_h)
...
class bn_mod_FiLM(nn.Module):
    def __init__(self, dataset_dictionary, n_cnn=4, n_resblock=4, conv_hidden=128, embed_hidden=200, gru_hidden=4096):
        ...
        self.res_lins = nn.ModuleList()
        for i in range(n_resblock):
            self.resblocks.append(ResBlock(conv_hidden, n_cnn, n_resblock))
