...
class RelNetConditionalBatchNorm(nn.Module):
    def __init__(self, dataset_dictionary, architecture_dictionary):
        super(RelNetConditionalBatchNorm, self).__init__()
        act_f = nn.ReLU()
        ...
        self.batch_norm_predict_layer = nn.Linear(ques_dim, 2)
    ...
    def forward(self, img, ques):
        ...
        conditional_batch_norm_param = self.batch_norm_predict_layer(ques)
        conditional_batch_norm_param = conditional_batch_norm_param.unsqueeze(2).unsqueeze(3).unsqueeze(4).repeat(1, 1, c, h, w)
        # apply the condiitonal batch norm parameters
        img_mean = torch.mean(img, dim = 0)
        img_variance = torch.std(img, dim = 0)
        img_norm = (img - img_mean) / (img_variance + 1e-10) # add epsilon for stability reasons
        img = conditional_batch_norm_param[:, 0].squeeze(1) * img_norm + conditional_batch_norm_param[:, 1].squeeze(1)
        img = self.img_net_last_layer(img)
        ##################################### Batch Norm logic ENDS #########################################################################
